...
import { configurationsModule, parserEntities } from "./helpers/runtime";
...
type EdiTransactionSetType = import("../dist/parser/entities").EdiTransactionSet;
type EdiSegmentType = import("../dist/parser/entities").EdiSegment;
type EdiElementType = import("../dist/parser/entities").EdiElement;

suite("edi-parser entities", () => {
  test("EdiComment and EdiSegment should expose formatting, nesting and element lookup helpers", () => {
    ...
    const component = createComponent(regularSegment, "0101", "AA");
  ...
  test("EdiElement should validate values, delegate composite errors and render identifiers", () => {
    ...
    const componentChild = createComponent(segment, "0101", "TOO-LONG");
    ...
    assert.deepStrictEqual(invalidErrors.map((error: any) => error.code), [
  ...
  test("EdiElement should honor non-service custom qualifier scopes and preserve missing releases", () => {
    ...
    assert.strictEqual(orphanErrors[0].others.release, undefined);
  });
  ...
  test("Transaction set, functional group, interchange and document should report structural errors", () => {
    ...
    assert.deepStrictEqual(functionalGroupErrors.map((error: any) => error.code), [
      DiagnosticErrors.INVALID_VALUE,
      DiagnosticErrors.INVALID_VALUE,
    ]);
    assert.deepStrictEqual(interchangeErrors.map((error: any) => error.code), [
      DiagnosticErrors.INVALID_VALUE,
      DiagnosticErrors.INVALID_VALUE,
    ]);
...
function createHierarchy(): {
  ...
  functionalGroup: EdiFunctionalGroupType;
...
function createSegment(id: string, startIndex: number, delimiter: string): EdiSegmentType {
  return new EdiSegment(id, startIndex, startIndex + id.length - 1, id.length, delimiter);
}

function createTrailer(id: string, count: string, controlId: string): EdiSegmentType {
...
function createElement(segment: EdiSegmentType, designatorIndex: string, value?: string): EdiElementType {
  const element = new EdiElement(segment, ElementType.dataElement, 1, value ? value.length : 0, "*", segment.id, segment.startIndex, designatorIndex);
  element.value = value;
  return element;
}

function createComponent(segment: EdiSegmentType, designatorIndex: string, value?: string): EdiElementType {
  const element = new EdiElement(segment, ElementType.componentElement, 1, value ? value.length : 0, ":", segment.id, segment.startIndex, designatorIndex);
  element.value = value;
  return element;
}
...
function createSchemaElement({
  ...
  return {
    ...
    getCodeOrNullByValue(value: string) {
      const found = qualifierCodes && qualifierCodes.find(code => code.value === value);
